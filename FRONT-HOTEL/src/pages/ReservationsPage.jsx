...
export default function ReservationsPage() {
  ...
  // ✅ Solo admin o cliente dueño puede editar/borrar
  const canManageReservation = (r) => {
    if (isAdmin) return true;
    if (isCliente && r.id_cliente === user?.id) return true;
    return false;
  };

  // ✅ Si es cliente, fijamos id_cliente en el formulario
  useEffect(() => {
  ...
  // ✅ Corrige automáticamente la fecha de salida si es anterior a entrada
  useEffect(() => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitError(
        e2?.response?.data?.message || "Error guardando la reserva"
      );
    }
  };

  const onEdit = (r) => {
    if (!canManageReservation(r)) return;
    setEditingId(r.id);
    setForm({
  ...
  const roomOptions = useMemo(() => {
    return [
      ...
      ...rooms.map((r) => ({
        value: String(r.id),
        label: `${r.tipo} — $${r.precio_noche} (cap ${r.capacidad}) · Hab ${
          r.numero_habitacion || r.id
        }`,
      })),
    ];
  }, [rooms]);

  // ✅ Filtrar reservas visibles según rol
  const visibleReservations = useMemo(() => {
    if (isAdmin) return reservations;
    if (isCliente) return reservations.filter((r) => r.id_cliente === user?.id);
    return [];
  }, [reservations, isAdmin, isCliente, user?.id]);
  ...
  return (
    <div className="p-6 rooms-page">
      ...
      <div className="card-elevated mb-6" style={{ padding: 14 }}>
        <form
          onSubmit={onSubmit}
          className="grid gap-3 rooms-form-grid"
          style={{
        ...
        >
          <div className="field">
            ...
            <NiceSelect
              ...
              options={roomOptions}
              onChange={(e) =>
                setForm((f) => ({ ...f, id_habitacion: e.target.value }))
              }
            />
          </div>
          ...
          {checking && (
            <div
              style={{ gridColumn: "1 / -1", color: "#94a3b8" }}
            >
          ...
          {conflict && (
            <div
              style={{ gridColumn: "1 / -1", color: "#fca5a5", fontWeight: 600 }}
            >
              Se encuentra agotado el alojamiento para estas fechas (
        ...
        {roomSelected && (
          <div className="mt-3 text-sm" style={{ opacity: 0.85 }}>
            Seleccionada: <strong>{roomSelected.tipo}</strong> — $
            {roomSelected.precio_noche} (cap {roomSelected.capacidad}) · Hab{" "}
            {roomSelected.numero_habitacion || roomSelected.id}
          </div>
        )}
      </div>
{/* Tabla */}
      ...
      <tbody>
        {visibleReservations.map((r) => (
          <tr key={r.id}>
            ...
            <td>
              {r.room
                ? `${r.room.tipo} — $${r.room.precio_noche} (cap ${r.room.capacidad}) · Hab ${r.room.numero_habitacion || r.id_habitacion}`
                : `Hab ${r.id_habitacion}`}
            </td>
            ...
            <td>{r.check_out}</td>
            <td>
              <span className="type-chip">{r.estado}</span>
            </td>
        ...
        {!visibleReservations.length && (
          <tr>
            <td colSpan={8} style={{ opacity: 0.75, padding: "8px 0" }}>
              Sin registros
            </td>
          </tr>
        )}
